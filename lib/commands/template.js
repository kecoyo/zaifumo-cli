    ...
    const startWork = () => {
      gulp
        ...
        .pipe(
          rename((parsed) => {
            // 替换文件名
            if (parsed.basename.includes(_.upperFirst(_.camelCase(srcName)))) {
              parsed.basename = parsed.basename.replace(_.upperFirst(_.camelCase(srcName)), _.upperFirst(_.camelCase(destName))); // 'FooBar'
            } else if (parsed.basename.includes(_.camelCase(srcName))) {
              parsed.basename = parsed.basename.replace(_.camelCase(srcName), _.camelCase(destName)); // 'fooBar'
            } else if (parsed.basename.includes(_.kebabCase(srcName))) {
              parsed.basename = parsed.basename.replace(_.kebabCase(srcName), _.kebabCase(destName)); // 'foo-bar'
            } else if (parsed.basename.includes(_.snakeCase(srcName))) {
              parsed.basename = parsed.basename.replace(_.snakeCase(srcName), _.snakeCase(destName)); // 'foo_bar'
            }
            return parsed;
          })
        )
